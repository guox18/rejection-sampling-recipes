...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="sft",
        help="输出文件名前缀，例如 train.jsonl -> sft_train.jsonl",
    )
    parser.add_argument(
        "--config", "-c",
        type=str,
...
def main():
    ...
    if args.output_dir:
        ...
        # 用户没有指定：在输入文件的父目录下创建 sft/YYYYMMDD_HHMMSS/
        # 对于多个输入文件，使用第一个文件的父目录
        first_input_dir = os.path.dirname(os.path.abspath(input_files[0]))
        output_base_dir = os.path.join(first_input_dir, "sft", timestamp)
    ...
    for i, (inp, out) in enumerate(zip(input_files, output_files), 1):
        print(f"  [{i}] {inp}")
        print(f"      → {out}")
    print()
