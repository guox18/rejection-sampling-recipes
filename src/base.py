    ...
    def _default_sync_process(self, batch: list[dict]) -> list[dict]:
        ...
        for item in batch:
            ...
            try:
                ...
                print(f"[{type(self).__name__}] Item {item.get('id', 'unknown')} failed: {e}")
                results.append({**item, "_failed": True, "_error": f"{type(self).__name__}: {e}"})
        return results
    ...
    def _default_threaded_process(self, batch: list[dict]) -> list[dict]:
        ...
        def safe_process_one(item):
            ...
            try:
                ...
                print(f"[{type(self).__name__}] Item {item.get('id', 'unknown')} failed: {e}")
                return {**item, "_failed": True, "_error": f"{type(self).__name__}: {e}"}
