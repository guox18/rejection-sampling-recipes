    ...
    for jsonl_file in sorted(jsonl_files):
        ...
        try:
            total_lines, cleaned_lines, error_lines = clean_resume_id_from_file(
                str(jsonl_file), str(output_file), verbose=verbose
            )
...
示例：
  ...
  python scripts/clean_resume_id_batch.py -i data/raw -o data/cleaned --quiet
    ...
    parser.add_argument("-i", "--input", required=True, dest="input_dir", help="输入目录路径")
    parser.add_argument("-o", "--output", required=True, dest="output_dir", help="输出目录路径")
    parser.add_argument("-q", "--quiet", action="store_true", help="安静模式，只显示摘要信息")
